...
- **@namespace/common**
  ...
  - [React](https://github.com/facebook/react) common library.
...
- **@namespace/react-app**
  ...
  - Uses the `@namespace/common` package (also inside monorepo).
...
Some packages depend on sibling packages within the monorepo. For example, in this repo, `@namespace/react-app` depends on `@namespace/common`. This relationship is just a normal dependency, and can be described in the `package.json` of `@namespace/react-app` like so:

```json
"dependencies": {
  "@namespace/common": "<package version>"
}
```
